...
#
# recommended pylint: pylint kinit.py -d maybe-no-member,line-too-long --indent-string "  "
# recommended formating: autopep8 --indent-size 2 -i --ignore E501 kinit.py
...
You have to specify user and password for kerberos
  ...
  - name: Refresh
    kinit: usr='root' pass='kerberos_password'
'''
  ...
  if std_o == '' or (err_o != None and err_o != ''):
    ...
    if match('^Default principal: {0}@.*$'.format(usr_a), std_lines[1]) == None:
      ...
      try:
        ...
        kerberos_renew = True
  if kerberos_renew:
    std_o, err_o = execute('echo -e "{1}\n" | sudo -u "{2}" kinit "{0}"'.format(usr_a, pass_a, usr_a.split('/')[0].split('@')[0]))
